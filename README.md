...
| `--filtered-port <n>` | none | A second port serving the scoped feed |
| `--save-dir <dir>` | none | Where the room persists itself |
...
| `--tls-cert <file.pem>` | none | Certificate chain; terminates TLS on the room port |
| `--tls-key <file.pem>` | none | Its private key. Both or neither |
...
`group`. All three doors into one produce the same flood of checks and the same
announcement to clients, so nothing downstream can tell a player giving up on
their own world from an organizer clearing one. Both are written *before*
...
thread does the JSON. The channel drops rather than blocks if a disk stalls,
...
| `PAHOA_SLOT_PASSWORDS` | none | A password per slot, as JSON |
...
[docs/slots.md](docs/slots.md) for which slots this covers. `--server-password`
is a third, orthogonal thing: it gates `!admin`, not joining, so it coexists
with either mode.
...
excluded (the reader task that sees those bytes has a connection id and no
slot), which makes this exactly the byte counterpart of `pahoa_packets_in_total`,
...
| option | default | |
|---|---|---|
| `--password <pw>` | none | Required from every client on connect |
| `--server-password <pw>` | none | Enables `!admin login`; unset refuses it outright |
